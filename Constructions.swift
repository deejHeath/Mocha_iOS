...
class Triangle: Measure { // parent: point, point, point, (unit) distance
    ...
    override func draw(_ context: CGContext, _ isRed: Bool) {
        context.setFillColor(UIColor(hue:(CGFloat(5*index)/23.0), saturation: 1.0, brightness: 1.0, alpha: 0.25).cgColor)
        ...
        if isRed {
            ...
            context.setStrokeColor(UIColor(hue:(CGFloat(5*index)/23.0), saturation: 1.0, brightness: 1.0, alpha: 0.5).cgColor)
        }
        context.setLineWidth(2.0)
        let currentRect = CGRect(x: coordinates.x-4.0,y:coordinates.y-4.0,
                                 width: 8.0,
